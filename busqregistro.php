...
if(!empty($_POST['nac'])){
 	...
 	if ($totalRows_rsRegist > 0) {
      $return = array('error'=>'Ya Esta Registrado en este Operativo','swformu'=>'1');
			...
			mysql_select_db($database_conn_registro, $conn_registro);
			$query_rsRep = "SELECT id, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, cod_centro FROM rep_sucre WHERE nacionalidad  = '$_POST[nac]' and cedula = $_POST[ced] ORDER BY cedula ASC";
          ...
          'centro_v' => $row_rsRep['cod_centro']
          );
				} else {
         
          $sql = "SELECT ctro_prop, nombre_centro from centro_votaciones where estado = 17 and  municipio = $_SESSION[MM_UserMun] and parroquia = $_POST[parroquia]";
          ...
          $array = ['error'=>'No se encuentra en el Registro Electoral del Circuito','swformu'=>'0','centros' => $data];
				  $return = $array;
			   }
